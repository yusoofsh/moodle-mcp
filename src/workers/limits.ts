...
export function reserveHttpRequest(
  store: SqlAuthStore,
  secret: string,
  address: string,
  path: string,
  ...
  const ip = createHmac("sha256", Buffer.from(secret, "hex"))
    ...
    .digest("hex");
  const budgets = [
    { key: "all", window: 60, limit: 240 },
    { key: `ip:${ip}`, window: 60, limit: 120 },
  ];
  ...
  const counters = budgets.map((b) => {
    ...
    return {
      ...b,
      count: valid ? Number(old.count) : 0,
      resetAt: valid ? Number(old.resetAt) : now + b.window,
    };
  });
  const retry = Math.max(
    0,
    ...counters.filter((b) => b.count >= b.limit).map((b) => b.resetAt - now),
  );
