...
export const connectionScript = String.raw`(() => {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const refresh = async () => {
      ...
      if (s.status === 'connected' && !result.pending && !confirmation) notice('A Moodle SSO account is saved. Use Check connection, then return to ChatGPT.');
    };
