...
async function start(override = {}) {
  const options = convertV4MiniflareOptions({
    ...
    outboundService: async (request) => {
      ...
      if (
        target.pathname ===
        "/webservice/pluginfile.php/72/mod_resource/content/handout.pdf"
      ) {
        assert.equal(target.searchParams.get("token"), bindings.MOODLE_TOKEN);
        return new Response(parserPdfFixture(), {
                  ...
                  contents: [
                    {
                      ...
                      fileurl:
                        "https://moodle.example/webservice/pluginfile.php/72/mod_resource/content/handout.pdf",
                    },
                  ],
                },
                {
...
try {
  ...
  await check(
    "authorized file download returns real PDF text through structured output",
    async () => {
      const list = await rpc(granted.access_token, "tools/call", {
        name: "moodle_list_resources",
        arguments: { courseId: 7 },
      });
      const fileId = /handout\.pdf[^\n]*fileId: `(f_[^`]+)`/.exec(
        list.json.result.structuredContent.text,
      )?.[1];
      assert.ok(fileId, list.text);
      for (const name of ["moodle_download_file", "moodle_read_document"]) {
        ...
        assert.notEqual(r.json.result.isError, true, r.text);
        assert.equal(
          r.json.result.structuredContent.data.document.status,
          "extracted",
        );
        assert.match(
          r.json.result.structuredContent.data.document.text,
          /Synthetic authenticated Moodle PDF/,
        );
        assert.ok(!r.text.includes(bindings.MOODLE_TOKEN));
      }
      const denied = await rpc(granted.access_token, "tools/call", {
