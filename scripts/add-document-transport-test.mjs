...
const path='scripts/test-workers.mjs';let s=await fs.readFile(path,'utf8');
if(s.includes('function parserPdfFixture'))process.exit(0);
s='import { strToU8 } from "fflate";\n'+s;
const pos=s.indexOf('const ORIGIN');if(pos<0)throw new Error('Missing runtime test origin');
s=s.slice(0,pos)+`function parserPdfFixture() {
...
`+s.slice(pos);
const route='      const target = new URL(request.url);';if(!s.includes(route))throw new Error('Missing request fixture route');
...
s=s.replace(modules,modules+`                {id:707,instance:807,name:'PDF handout',modname:'resource',uservisible:true,contents:[{type:'file',filename:'handout.pdf',filepath:'/',filesize:900,mimetype:'application/pdf',fileurl:'https://moodle.example/webservice/pluginfile.php/72/mod_resource/content/handout.pdf'}]},
`);
const index=s.indexOf('  await check(\n    "authenticated discovery survives Moodle failure');if(index<0)throw new Error('Missing discovery test location');
s=s.slice(0,index)+`  await check('authorized file download returns real PDF text through structured output',async()=>{
